    ...
    public void serverProcess(@NotNull UserBase user, @NotNull TNTServer server) {
        if (user instanceof Player player) {
            ...
            HypixelCache.get(player.getUuid(), userList,
                    playersReady -> user.sendPacket(new ReceiveHypixelPlayerV2Packet(playersReady, false)),
                    () -> user.sendPacket(new ReceiveHypixelPlayerV2Packet(Collections.emptyMap(), true)));
        } else {
            user.disconnect();
        }
    }
